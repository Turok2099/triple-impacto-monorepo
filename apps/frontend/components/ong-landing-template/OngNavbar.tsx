    ...
    <nav className="fixed top-0 left-0 right-0 z-50 bg-white/90 backdrop-blur-md border-b border-gray-100 shadow-sm">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <a
            href="#"
            className="flex items-center gap-2 text-xl font-bold text-teal-700 hover:text-[#2c8184] transition-colors"
          >
          ...
          <div className="hidden md:flex items-center gap-8">
            {navLinks.map((link) => (
              <a
                ...
                className="text-gray-600 hover:text-[#2c8184] font-medium transition-colors"
              >
                {link.label}
              </a>
            ))}
          ...
          {/* Mobile Menu Button */}
          <button
            className="md:hidden p-2 text-gray-600 hover:text-[#2c8184] focus:outline-none focus:ring-2 focus:ring-emerald-500 rounded-lg"
            ...
            <div className="flex flex-col gap-3">
              {navLinks.map((link) => (
                <a
                  ...
                  href={link.href}
                  onClick={(e) => handleScrollTo(e, link.href)}
                  className="px-4 py-2 text-gray-600 hover:text-[#2c8184] hover:bg-teal-50 rounded-lg font-medium transition-colors"
                >
                  {link.label}
                </a>
              ))}
