...
export default function SeccionAdminOngs() {
  ...
  const handleCopyLink = async (url: string, key: string) => {
    try {
      ...
      Swal.fire("Error", "No se pudo copiar el link correctamente", "error");
    }
  };
  ...
  const handleDeactivate = async (id: string, name: string) => {
    ...
    try {
      ...
      Swal.fire(
        "¡Desactivada!",
        "La ONG ha sido ocultada del sitio.",
        "success",
      );
      fetchOngs();
    } catch (err: any) {
  ...
  const handlePermanentDelete = async (id: string, name: string) => {
    ...
    try {
      ...
      Swal.fire(
        "¡Eliminada!",
        "La ONG ha sido borrada completamente del sistema.",
        "success",
      );
      fetchOngs();
    } catch (err: any) {
  ...
  const handleToggleStatus = async (ong: Ong) => {
    try {
      ...
      setOngs(
        ongs.map((o) => (o.id === ong.id ? { ...o, activa: newStatus } : o)),
      );
  ...
  const openEditModal = (ong: Ong) => {
    ...
    setPreviewUrl(null);
    const bonda =
      ong.bonda_microsites && ong.bonda_microsites.length > 0
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingOng) {
        ...
        Swal.fire(
          "¡Actualizada!",
          "La ONG fue modificada con éxito.",
          "success",
        );
      } else {
  ...
  if (error) {
    return (
      <div className="bg-red-50 text-red-600 p-6 rounded-2xl border border-red-100 flex items-center gap-3">
        ...
        <span>Error cargando ONGs: {error}</span>
      </div>
    );
  }
        ...
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-slate-100">
            {ongs.map((ong) => {
              ...
              const bondaUrl = bonda
                ? bonda.slug.startsWith("http")
                  ? bonda.slug
                  : `https://${bonda.slug}`
                : null;
              ...
              return (
                <tr
                  key={ong.id}
                  className={`transition-colors ${fiservOk && ong.activa ? "bg-blue-50/60 hover:bg-blue-50" : "hover:bg-slate-50/50"}`}
                >
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-500 line-clamp-2 max-w-[200px]">
                          {ong.descripcion || "Sin descripción"}
                        </p>
                      </div>
                    </div>
                  </td>
                      ...
                      <div className="flex items-center gap-1 text-xs">
                        {bonda && bondaUrl ? (
                          <>
                            ...
                            <button
                              onClick={() =>
                                handleCopyLink(bondaUrl, `bonda-${ong.id}`)
                              }
                          ...
                          <span className="bg-red-50 text-red-500 px-2 py-0.5 rounded font-medium">
                            Sin Bonda
                          </span>
                        )}
                      </div>
                        ...
                        <div className="flex items-center gap-1 text-xs">
                          ...
                          {!fiservOk && (
                            <span
                              ...
                              title="El link ya existe pero todavía no procesa pagos: falta completar Fiserv (Store ID + Shared Secret) y activarlo."
                            >
                              ⚠ inactivo
                            </span>
                          )}
                    ...
                    <div className="flex flex-col gap-3">
                      <div className="flex items-center justify-between">
                        ...
                        >
                          ...
                          <span
                            className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${ong.activa ? "translate-x-6" : "translate-x-1"}`}
                          />
                        </button>
                      </div>
                    </div>
                  ...
                  <td className="py-4 px-6 text-right">
                    <div className="flex justify-end gap-2">
                      ...
                      <button
                        onClick={() =>
                          handlePermanentDelete(ong.id, ong.nombre)
                        }
                        className="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors"
                        title="Eliminar ONG permanentemente"
                      >
            ...
            >
              <div className="space-y-8">
                ...
                <section>
                  <h3 className="text-lg font-bold text-slate-800 border-b border-slate-200 pb-2 mb-4 flex items-center gap-2">
                    <Building2 className="w-5 h-5 text-slate-400" /> Información
                    Pública
                  </h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <h4 className="col-span-2 text-sm font-bold text-slate-700 mt-2 mb-1">
                    ...
                    <div className="col-span-2">
                      <label className="block text-sm font-semibold text-slate-700 mb-1">
                        Slug de Donación Exclusivo (amigable)
                      </label>
                      <input
                        placeholder="ej: fundacion-padres (dejar vacío para deshabilitar exclusive link)"
                        value={formData.slug}
                        onChange={(e) =>
                          setFormData({
                            ...formData,
                        ...
                        onBlur={(e) =>
                          setFormData({
                            ...formData,
                            slug: e.target.value.replace(/^[-_]+|[-_]+$/g, ""),
                          })
                        }
                        className="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:ring-2 focus:ring-emerald-500 outline-none"
                      />
                    ...
                    <div className="col-span-2">
                      <label className="block text-sm font-semibold text-slate-700 mb-2">
                        Logo de la Organización (JPG/PNG)
                      </label>
                      <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-xs text-slate-500 mt-2 font-medium">
                            La imagen se subirá automáticamente a Supabase al
                            guardar.
                          </p>
                        </div>
                      </div>
                ...
                <section>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setFormData({
                            ...formData,
                            monto_fijo_1: Number(e.target.value),
                          })
                        }
                        className="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:ring-2 focus:ring-emerald-500 outline-none"
                      />
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setFormData({
                            ...formData,
                            monto_fijo_2: Number(e.target.value),
                          })
                        }
                        className="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:ring-2 focus:ring-emerald-500 outline-none"
                      />
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setFormData({
                            ...formData,
                            monto_fijo_3: Number(e.target.value),
                          })
                        }
                        className="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:ring-2 focus:ring-emerald-500 outline-none"
                      />
                  ...
                  <div className="bg-orange-50/50 p-4 rounded-xl border border-orange-100 flex flex-col gap-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-sm font-semibold text-orange-950 mb-1">
                          Fiserv Store ID
                        </label>
                        <input
                          placeholder="Sin información"
                          autoComplete="off"
                          value={formData.fiserv_store_id}
                          onChange={(e) =>
                            setFormData({
                              ...formData,
                              fiserv_store_id: e.target.value,
                            })
                          }
                          className="w-full px-4 py-2.5 rounded-lg border border-orange-200 focus:ring-2 focus:ring-orange-500 outline-none font-mono text-sm"
                        />
                      </div>
                      <div>
                        <label className="block text-sm font-semibold text-orange-950 mb-1">
                          Fiserv Shared Secret
                        </label>
                        <div className="relative">
                          <input
                            placeholder={
                              editingOng?.fiserv_shared_secret_configurado
                            ...
                            onChange={(e) =>
                              setFormData({
                                ...formData,
                                fiserv_shared_secret: e.target.value,
                              })
                            }
                            className="w-full px-4 py-2.5 pr-10 rounded-lg border border-orange-200 focus:ring-2 focus:ring-orange-500 outline-none font-mono text-sm"
                          />
                          <button
                            type="button"
                        ...
                        <p className="text-xs text-slate-500 mt-1">
                          Por seguridad no se muestra el valor guardado.{" "}
                          {editingOng?.fiserv_shared_secret_configurado
                            ? "Ya hay uno configurado; escribí uno nuevo solo si querés reemplazarlo."
                    ...
                    <div className="flex items-center gap-3 mt-2 border-t border-orange-200/50 pt-4">
                      <input
                        type="checkbox"
                        id="fiserv_activo"
                        checked={formData.fiserv_activo}
                        onChange={(e) =>
                          setFormData({
                            ...formData,
                            fiserv_activo: e.target.checked,
                          })
                        }
                        className="w-5 h-5 text-orange-600 rounded border-orange-300 focus:ring-orange-500"
                      />
                      <label
                        htmlFor="fiserv_activo"
                        className="font-semibold text-orange-950"
                      >
                        Fiserv Activo (Habilita la ONG en el formulario de pago,
                        requiere Store ID y Shared Secret)
                      </label>
                    </div>
                    {formData.fiserv_activo && !formData.fiserv_store_id && (
                      <p className="text-xs text-red-600 font-medium -mt-2">
                        Falta el Store ID para poder activar Fiserv.
                      </p>
                    )}
                    {formData.fiserv_activo &&
                      !formData.fiserv_shared_secret &&
                      !editingOng?.fiserv_shared_secret_configurado && (
                        <p className="text-xs text-red-600 font-medium -mt-2">
                          Falta el Shared Secret para poder activar Fiserv.
                        </p>
                      )}
                  </div>
                </section>
                ...
                <section>
                  ...
                  <div className="bg-rose-50/50 p-4 rounded-xl border border-rose-100 grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="col-span-2">
                      <label className="block text-sm font-semibold text-rose-950 mb-1">
                        URL ONG + BONDA
                      </label>
                      <input
                        placeholder="ej: https://beneficios-mi-ong.bonda.com"
                        value={formData.bonda_slug}
                        onChange={(e) =>
                          setFormData({
                            ...formData,
                            bonda_slug: e.target.value
                              .trim()
                              .replace(/\/+$/, ""),
                          })
                        }
                        className="w-full px-4 py-2.5 rounded-lg border border-rose-200 focus:ring-2 focus:ring-rose-500 outline-none"
                      />
                    </div>
                    <div>
                      <label className="block text-sm font-semibold text-rose-950 mb-1">
                      ...
                      <p className="text-xs text-rose-600 mb-2">
                        Usada para consultar el catálogo de beneficios.
                      </p>
                      <div className="relative">
                        <input
                          type={showSecrets ? "text" : "password"}
                          ...
                          onChange={(e) =>
                            setFormData({
                              ...formData,
                              bonda_api_token: e.target.value,
                            })
                          }
                          className="w-full px-4 py-2.5 pr-10 rounded-lg border border-rose-200 focus:ring-2 focus:ring-rose-500 outline-none font-mono text-sm"
                        />
                        <button
                          type="button"
                          onClick={() => setShowSecrets(!showSecrets)}
                    ...
                    <div>
                      <label className="block text-sm font-semibold text-rose-950 mb-1">
                        API Key de Nóminas (Afiliados)
                      </label>
                      <p className="text-xs text-rose-600 mb-2">
                        Usada para crear/eliminar afiliados desde nuestro
                        sistema.
                      </p>
                      <div className="relative">
                        <input
                          ...
                          onChange={(e) =>
                            setFormData({
                              ...formData,
                              bonda_api_token_nominas: e.target.value,
                            })
                          }
                          className="w-full px-4 py-2.5 pr-10 rounded-lg border border-rose-200 focus:ring-2 focus:ring-rose-500 outline-none font-mono text-sm"
                        />
                    ...
                    <div className="col-span-2">
                      ...
                      <input
                        placeholder="ID numérico si aplica"
                        value={formData.bonda_microsite_id}
                        onChange={(e) =>
                          setFormData({
                            ...formData,
                            bonda_microsite_id: e.target.value,
                          })
                        }
                        className="w-full px-4 py-2.5 rounded-lg border border-rose-200 focus:ring-2 focus:ring-rose-500 outline-none font-mono text-sm"
                      />
                    </div>
                  </div>
                </section>
              ...
              <div className="mt-8 flex gap-3 pt-6 border-t border-slate-100">
                <button
                  type="button"
                  onClick={() => setIsModalOpen(false)}
                  className="flex-1 py-3 px-4 bg-white border border-slate-200 text-slate-700 hover:bg-slate-50 font-semibold rounded-xl transition-colors"
                >
                  Cancelar
                </button>
                <button
